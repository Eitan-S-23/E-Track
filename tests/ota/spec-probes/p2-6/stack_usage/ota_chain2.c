...
/* 插桩构型额外引入的观测调用。它本身很小，但足以改变父函数的内联决策。 */
extern void P2_6_note(unsigned tag);
#define P2_6_NOTE(tag) P2_6_note(tag)
#else
...
/* 第二调用点：干跑校验，不写入。与 ota_apply 共用 ota_stage_verify。 */
int ota_dry_run(unsigned nblk)
{
    unsigned char scratch[128];
    int rc = ota_stage_verify(nblk);
    scratch[0] = (unsigned char)rc;
    ota_sink_bytes(scratch, sizeof(scratch));
    return rc;
}
