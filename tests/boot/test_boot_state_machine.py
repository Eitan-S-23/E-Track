#!/usr/bin/env python3
"""Compile and run the P1-3 persistent Boot state-machine simulator."""
...
def compile_test(output: Path) -> None:
    sources = [
        ROOT / "tests/boot/test_boot_state_machine.c",
        ROOT / "boot/src/boot_state_machine.c",
...
def main() -> int:
    build_dir = ROOT / ".cache" / "p1-3-state-machine"
    build_dir.mkdir(parents=True, exist_ok=True)
    executable = build_dir / (
        "test_boot_state_machine.exe" if os.name == "nt"
        else "test_boot_state_machine"
    )
