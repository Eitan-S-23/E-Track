...
本节描述正式验收。设备开发批次另遵守 §7.4；这里的配额指用户明确上限或本轮已审批
合同中的真实执行计划，不引入全项目默认“一次 OTA/一次复位”或固定三轮上限。历史一次性
控制器的限额不是后续任务的永久授权边界；不得据此反复请示同一已批准动作。
...
3. **批量整改**：实现者对同根因的范围内调用点、配置与测试一起检查和修复，再交付一个
   ...
   单测、静态检查、增量编译可随修随跑；正式观测和硬件动作无论叫调试还是自测，都须
   符合现行用户授权和执行计划。设备开发批次按 §7.4 连续执行，用户明确上限仍须遵守。
   新增范围或安全风险先裁定，普通范围内缺陷不自动暂停整卡。
...
Flutter 调试与取证经验见 `.agents/skills/e-track-flutter-debug/SKILL.md`。该 skill 本身不
授予 CI/真机权限，也不改变历史冻结合同；新规范及 helper 使用宿主回归，不重开历史产品卡。
项目级共享入口为 `docs/agent-collaboration/index.md`，覆盖全部组件和任务；活动记录不进入
验收输入 profile，规范性规则仍在对应合同中维护。
...
用户于 2026-09-22 要求放宽 OTA 策略并持续执行到真正需要介入。新设备开发批次遵守
`docs/device-experiment-policy.md`：先记录有限实验矩阵、输入身份、恢复路线、单次超时、
最终保留状态与实际动作账目，再连续执行已授权范围。不默认设置一次 OTA、一次复位、
三轮或整任务 30 分钟上限；同范围候选筛选、必要恢复和有依据的局部复测不逐项请示。
有界宿主窗口可以由 agent 在确认旧实例关闭后续开，不因此重装或重复已成功的 OTA。

结果不明先查状态；完整性/恢复异常、新风险、新外部写入或用户明确上限才暂停相关动作。
其他安全工作继续。当前 P3-4 的 3.2.6 对照恢复已有条件授权，但须证明必要性并验证完整
App/BCB/持久状态恢复方案，不能只刷 App。普通 CI 授权仍不自动授权真机或发布。

此决定只改变今后执行节奏，不回改旧账本/失败回执/冻结包，也不放宽协议、安全或性能
判据。已有正式合同需改变执行计划时产生后继版本；本节不把开发测试升级为独立验收。
GitHub 产物存储与删除策略仍按 §7.3.3，不与物理 OTA 实验混同。
