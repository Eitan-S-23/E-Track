#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""P3-3 受控固件 OTA v2 测试服务（冻结源码，版本 1）。
...
class V2ServiceState:
    ...
    def handle_latest(self, query_pairs):
        ...
        if params['bootVersion'] < release.min_boot_version:
            raise ServiceError(
                ...
                releaseId=release.release_id)
        if params['protocolVersion'] < release.min_protocol_version:
            raise ServiceError(
    ...
    def handle_download(self, query_pairs, range_header, if_range_header):
        ...
        asset = release.asset
        if asset.asset_id != token['assetId'] or asset.kind != token['kind']:
            raise ServiceError(
    ...
    def do_GET(self):
        ...
        try:
            if parts.path == LATEST_PATH:
                status, headers, body = self.state.handle_latest(query_pairs)
            ...
            # 状态层携带的 requestId 优先（与状态层直调路径一致）；
            # 兜底用本层生成的 requestId，保证头/体一致。
        ...
        self.log.info('%s %s -> %d (%d bytes) req=%s',
                      self.command, self.path, status, len(body), request_id)
