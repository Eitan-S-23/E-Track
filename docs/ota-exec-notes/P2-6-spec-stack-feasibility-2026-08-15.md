...
**远端首轮的跨平台风险（撰写时的预先声明；已由 §9.4 实测消解）**：本轮全部探针
数值基线在 Windows + MinGW 宿主 gcc 15.2.0 上取得，`wrap_probe` 的
`real_pool_seq=(163,96)`、`scan4` 的 `ota_apply` 992B→520B 等精确数值要到 Ubuntu
runner 首跑才能确认。同为 13.3.Rel1 时 ARM 侧代码生成应一致，但**若远端首轮红，
处置口径是「工具链或构型已变 → 重新裁定基线」，不是把探针判为坏掉、也不是把
Spec 结论作废**。
...
Acceptance Governance 在 Ubuntu 上取得正式结果（PR #3 / run `31948667013` /
候选提交 `68b929b`，2026-08-16）：

| 步骤 | 远端实测输出 |
| --- | --- |
| Run governance regression tests | `Ran 45 tests ... OK`（**0 跳过**；本机 1 跳过为 Windows symlink 权限） |
| Run P2-6 spec probes | harness 自检 **20/20** 逐项 PASS + 探针 **8/8 PASS**，`rc=0` |

远端工具链：`arm-none-eabi-gcc 13.3.1 20240614`（Arm GNU Toolchain 13.3.Rel1
Build arm-13.24，与 `firmware-build.yml` 同版本）+ 宿主
`gcc (Ubuntu 13.3.0-6ubuntu2~24.04.1)`。

三点结论（都是**实测**，不是推定）：

1. §9.2 末尾预告的风险数值全部在 Ubuntu 上复现 —— 探针是 fail-closed 的（记录
   结论冻结成期望值），`8/8 PASS` 即意味着 `wrap_probe` 的
   `real_pool_seq=(163,96)`、`scan4` 的 `ota_apply` 992B→520B 与链上求和更深等
   基线在两个平台上一致。**无需重新裁定基线。**
2. 宿主编译器跨了两个大版本（MinGW 15.2.0 → Ubuntu 13.3.0）而 `host_scan` 的
   `S1..S5` 期望值不变，说明宿主侧判据（扫描算法与 guard 相互作用）不依赖具体
   宿主 gcc 版本；ARM 侧则仍**必须**锁 13.3.Rel1，那些是逐字节基线。
3. 远端跑到的测试比本机多一项（本机 symlink 权限跳过的那项在 Ubuntu 上实际
   执行并通过），因此「本机 45 通过 1 跳过」不是远端结果的上界。

据此，2026-08-16 定向复核第二轮裁定的前置条件「先取得含 8 组探针的正式远端结果
通过，之后才可冻结实现提示词」已满足。
