...
- 本表**未执行**任何设备操作：无安装、无烧录、无 BLE 连接、无 logcat 采集、无 RTT。
  （B1-M 于 2026-09-11 按本表 §7 授权另行执行完毕，对账与边界审计见 §6.5；
  该执行中的 `logcat -c`、设备文件覆盖、`am force-stop` 超出本表边界，已在 §6.5.3 披露。）
...
| T1b / B1-M 最小机制验证（广播 + 连接 + 三策略注错） | **三次写额度已用尽**（2026-09-11 执行完毕；对账与边界审计见 §6.5） |
...
| **B1-M（T1b 机制）** | **3（每策略 1 次写）** | **3** | **0** | 2026-09-11 r9/r20/r21 三次真实写各一次，额度已用尽（对账见 §6.5）；仅判机制、不判协议；**不占用也不重置** A1 额度 |
| B1（T1b 完整协议仿真） | 0（未授权） | 0 | — | 只在 B1-M 第 3 策略成立后才申请 |
| B2（T1b） | 0（未授权） | 0 | — | 同上 |

### 6.5 B1-M 执行对账与边界审计（2026-09-13 补记，OPS-01）

本节由 2026-09-12 集中整改（OPS-01）补写，对 2026-09-11 B1-M 实际执行做**事后对账**：
只引用既有原始产物，不重写任何日志；影响裁定留给非实现会话。

#### 6.5.1 额度对账（三次真实写）

交付报告（`P3-3-b1m-minimal-mechanism-2026-09-11.md` §0/§5）与原始
`peripheral.jsonl` 均证实 r9/r20/r21 各发生**一次**真实写（`writes_seen=1`、
`exit_code=0`）：立即应答 r9、延迟应答 N=3s r20、不应答 H=20s r21。
§6.4 已据实改为**已用 3、剩余 0**；旧表「已用 0/剩余 3」是执行前快照，未回写，已作废。

#### 6.5.2 全部尝试与整段时长（零写轮不抹除）

原始产物目录（`.cache/p3-3-t1b/runs/`）共 25 个运行目录。按 `peripheral.log` 落盘
时间重建的整段会话时间线（本机墙钟，只用于跨度核算）：

| 段 | 轮次 | 窗口 | 说明 |
| --- | --- | --- | --- |
| 不计额探测 | b1m-smoke / b1m-loopback-imm / b1m-adv-probe | 19:58–20:07 | 无手机接入，`writes_seen=0` |
| immediate 失败轮 | r1–r8 | 20:21–21:28 | r1 为 ENV_BLOCKED（适配器消失）；r2–r8 harness `static_value` 缺陷致零写 |
| immediate 有效轮 | r9 | 21:44 | **写 1 次，通过** |
| 中途探测 | b1m-setup-probe-fixed | 21:40 | 不计额 |
| delayed 失败轮 | r10–r19 | 21:52–22:33 | 界面导航失败、广播不可见（遗留 ACL）、脚本无输出、自动化未命中目标行 |
| delayed/withhold 有效轮 | r20 / r21 | 22:35 / 22:40 | **各写 1 次，通过** |

- **从 r1（20:21）到 r21（22:40）整段会话跨度约 2 小时 20 分**；交付报告「约 58 min」
  指的是 r9→r21 段，亦如实保留。两种口径均**远超** §7.1 事前固定的「三策略合计
  ≤ 20 分钟」。
- §7.1 时限判据的事前口径是「单策略 ≤ 5 min（`--total-limit`）」，三次有效轮 harness
  实测 167.2s/91.2s/250.9s 均满足；「三策略合计 ≤ 20 min」按交付报告的核算方式
  （只计三次有效轮 harness 时长 509.3s）勉强满足，但**把失败轮排除在合计之外没有
  事前授权依据**——该合计口径按未满足「含广播、连接、收尾」的原始表述处理，
  详见 §6.5.4。
...
| 副作用 | 轮次 | 与操作单边界的出入 |
| --- | --- | --- |
| `adb logcat -c` 清空手机日志缓冲 | 多轮（每次采集前） | **操作单 §4 明令「不清设备既有日志（不执行 logcat -c）」**；该边界写在 T1a 命令段，B1-M 执行时被违反，**未找到覆盖此项的另行批准锚点** |
| `uiautomator dump /sdcard/ui.xml` 覆盖手机同名文件 | r10 等含 UI 导航轮（r1/r8/r10/r19 等目录均有 ui-*.png / ui-dump.xml） | Android 标准落点，非项目产物；操作单未授权设备文件覆盖 |
| `am force-stop com.wen.gaia.gaia` 停止既有应用 | r11–r17 失败期（释放本机自建 GATT 客户端） | 操作单未授权停止既有应用；交付报告 §7 未将其计入副作用披露，本表补记；原始产物目录未见该命令的脚本/日志痕迹，仅有交付报告 §4.4 自述 |
| 手机侧 UI 自动化（`.cache/p3-3-t1b/b1m_connect.py`）：`monkey -p com.wen.gaia.gaia` 拉起应用、`input swipe` 点击/滚动 | r1–r21 多轮（遥控页导航与发送） | 操作单未授权输入注入/应用拉起；脚本原文即证据 |
| `screencap` 截图（`ui-*.png`，落盘主机侧） | 多个失败轮目录（r1/r5–r8/r10/r19 等） | logcat 含 `app_apk:screencap` 痕迹；截图经主机保存，设备端无残留写入，但截屏动作本身未授权 |
| 客户端 APK 1.0.60（release，非 dev CI 产物） | **既有安装，非本会话动作**：r1 `identity.md` 记录 `firstInstall 2026-06-30 / lastUpdate 2026-07-10`，B1-M 会话（2026-09-11）未执行 `adb install` | 无安装副作用；仅记录在机应用身份 |

以上均为 2026-09-11 已发生的历史事实，本轮（2026-09-12 整改批次）**没有新增任何
真机操作**；后续 B1/B2 或其它设备观测前，须按任务书要求针对每类动作另行申请授权。

#### 6.5.4 授权来源审计（缺锚点标缺失，不追认）

- **B1-M 主授权**：用户 2026-09-11 决定「批准广播、连接与三种策略的最小验证，
  范围仅限已核对的 PC 蓝牙适配器与本项目专用测试手机；新增额度为三种策略各一次
  实际写观测；不占 T1a 额度」。锚点：操作单 §7（授权口径段）与准备记录
  `P3-3-b1m-prep-2026-09-11.md` 首部。**原始聊天记录原文未入库**，仓库内锚点
  为二手转述——按「缺原授权原文/锚点则标明缺失」处理：转述锚点存在，原文缺失。
- **用户开启手机免打扰**：交付报告 §7 称「用户按授权开启手机免打扰」，同上仅有
  转述，无原文锚点。
- **`logcat -c` / 设备文件覆盖 / `am force-stop` / UI 自动化（monkey、input、screencap）**：
  **均未找到授权锚点**（既无原文也无转述）。本节仅如实记录已发生事实与边界出入，**不编造批准、不把
  2026-09-12 整改授权用于追认**；是否作废受影响证据由非实现会话裁定。
- 三次有效写的**机制样本保留**（原始日志不可重写）：其「旧 release 客户端
  `writeByAddress` 透传路径、非 OTA 专用写路径」的局限已在交付报告 §6 声明，
  不因本节审计升级或降级其证据地位。
...
| 三策略合计 | **≤ 20 分钟** | 同上（事前口径；执行对账见 §6.5.2：合计只计三次有效轮无事前依据，整段含失败轮跨度远超此限） |
