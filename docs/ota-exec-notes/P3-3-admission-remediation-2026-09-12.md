...
| BCB/镜像不一致 | **已升为实机前置硬阻断（用户第 2 条）**：BCB 20801 vs 镜像 30200 使 `ota_backup.c:385` 拒绝一切 STAGED 提交；最小恢复方案待裁定，不删检查/不改数字/不擅烧 | `P3-3-bcb-blocker-clarification-2026-09-13.md` |
| 可启动 toy 固件包 | **方案已落盘待执行**：GCC→finalize 3.2.1→pack-full 离线制包（用户第 4 条授权项目内准备与离线验证；不写设备）；golden vectors 不具上板资格 | `P3-3-asset-service-apk-plan-2026-09-13.md` §2 |
| 真包 .etu | 同上，3.2.2 独立封包，身份四元组分开记录 | 同上 §2.2 |
| 受控 HTTP 服务 | **方案已落盘待批**：A 复用 staging+fixture（需 D1 写入授权）/ B 全新实例；服务本身仍活着（公开端点探测通过） | 同上 §4 |
| 受验 APK endpoint | **当前 APK 无固件 endpoint**（run 日志证实 not configured）——注入方式三选一（仓库 variable / dev APK / build.yml dispatch 输入）待裁，选定后需追加 build-only 运行数 | 同上 §3 |
...
| 2. BCB/镜像身份问题澄清 | 不一致证据链、为何更高版本绕不过 385、恢复方案 A/B 待裁定 | **澄清已落盘，恢复方案待裁定** | `P3-3-bcb-blocker-clarification-2026-09-13.md` |
| 3. 两类安全资产及服务/APK 方案确定 | 版本占用检查通过；toy 3.2.1/真包 3.2.2 制包流程、endpoint 注入三选一、服务方案 A/B、5 项待裁定清单 | **方案已落盘待批** | `P3-3-asset-service-apk-plan-2026-09-13.md` §6 |
| 4. 必要准备和自测 | 资产构建+离线验证（项目内，不写设备）；受验 APK 构建（需 endpoint 方案先定，可能追加 build-only 配额）；源码整改是否全部关闭由独立复核确认 | 待授权执行 | 同上 §2/§3 |
...
| 6. 用户审批 | 审批冻结合同 + 合并操作单（§11） | 待前置阶段完成 | §11 |
| 7. 冻结及 NOT_RUN 前检 | `validate_bundle.py`（FROZEN + NOT_RUN）执行前检 | 待审批 | — |
| 8. 分阶段实机执行 | APK 安装→toy 闭环→真包闭环（BCB 阻断处置贯穿）；每阶段证据回填矩阵 | 待全部前置完成 | §11 |
...
## 11. 合并操作单框架（待用户集中审批；本节是框架，不是已获授权）

用户第 7 条要求：真机授权逐项、最小范围；先提交一份合并操作单集中审批。
本节为框架草案，**全部操作均未授权、未执行**。

### 11.1 设备与资产身份（执行前须逐项绑定实物哈希）

| 项 | 身份 | 状态 |
| --- | --- | --- |
| 测试手机 | vivo V2312A，序列号 `10ADA4197U001CK`，Android 13（SDK 33）；既有安装 `com.wen.gaia.gaia` 1.0.60(86) | §6.2 |
| 目标板 | AT32F435RGT7，板上 App 3.2.0(30200)，Boot 与磁盘 `X-Track-Boot.bin` 一致；BCB CONFIRMED 20801（阻断中） | §6.1 |
| 受验 APK | 待 endpoint 方案定后构建；绑定包名/versionCode/证书指纹/APK SHA-256 | §3、资产方案 §3 |
| toy 包 | toy-3.2.1(30201) .etu + 目标镜像，四元组 | 资产方案 §2 |
| 真包 | real-3.2.2(30202) .etu + 目标镜像，四元组 | 资产方案 §2 |

### 11.2 操作序列（每项含命令/超时/次数/失败处理，执行前细化进合同）

| # | 操作 | 工具/命令（示意） | 超时 | 次数 | 失败/冲突处理 |
| --- | --- | --- | --- | --- | --- |
| O1 | APK 安装（先核对签名与既有安装是否一致） | `adb install -r <APK>`；`keytool -printcert -jarfile`、`adb shell dumpsys package` 留证 | 5 min | 1 | 签名冲突 → **不卸载/不清数据**，停报用户裁定 |
| O2 | APK 启动 + 对受控服务真实 latest/下载请求观测 | App 内观测插桩日志 + 服务端请求日志 | 10 min | 1 | 网络不可达 → 记 ENV_BLOCKED，不动设备 |
| O3 | toy 3.2.1 闭环：App→BLE→MCU 传输→STAGED→Commit→重启→GET_INFO 比对 | App 操作 + RTT/J-Link **只读**采集（若需） | 30 min/窗 | 1 | BCB 385 拒绝为预期首观测（澄清文档 §4 方案 A）→ 停，报用户裁定是否进方案 B |
| O4 | 真包 3.2.2 闭环（同 O3，目标 30202/独立镜像身份） | 同上 | 同上 | 1 | 同上；不得由 toy 结果代替 |
| O5 | 验收后只读身份复核 | `GET_INFO`、`adb` 只读 | 5 min | 1 | — |

### 11.3 明确**不含**的操作（须单独列项审批才可能执行）

- 断电注错实验：两个正常成功闭环不默认需要；确需时单独说明原因列项。
- J-Link 烧录：同上；且**可能触发复位的 J-Link 连接不作为免费只读预检**——
  O3/O4 中若需 RTT 采集，连接行为本身须在操作单中列为独立项。
- 额外 B1-M 实验（PC 蓝牙适配器路径）：同上。
- 额度互借：**A1、B1-M 与升级闭环额度不得互借**。

### 11.4 恢复方案

- O1 签名冲突：停在安装前，保留既有安装与数据，报用户。
- O3/O4 传输失败：设备保持 STAGED/断点状态（产品自愈语义），不盲重试；
  按 `MONO_*` 插桩留证后分类 PRODUCT_FAIL/HARNESS_FAIL。
- O3 BCB 385 拒绝（预期）：记录实机证据后停止，等待用户裁定恢复路径
  （方案 B TEST_BOOT 属真机写入，须另批）。
- 全程不写 BCB、不烧录、不清手机数据。
