...
本文件只记录决策过程、责任人和落盘目标，不是接口真相源。接口裁定写入对应领域合同；任务边界裁定同步到看板和受影响 Spec，不改写冻结二进制或历史验收合同。用户在记录 9 中批准 OTA-DEC-001 至 OTA-DEC-012 并授权冻结规范；OTA-DEC-013 依据记录 10 的单独授权裁定，OTA-DEC-014 依据记录 11 的新增需求落盘授权。用户原话与参数细化保留在下文。
...
## OTA-DEC-013 P3-3 与 P3-5 实机验收归属和完成时点

- 问题描述：P3-3 卡要求 Actions 构建绿、APK 可装、真机 toy/真包传输成功，实施 Spec 却把安装和传输推迟到依赖 P3-3 的 P3-5；该矛盾已在派单就绪记录登记为 R2，但此前没有迁移或豁免授权。
- 所属领域：任务治理 / Flutter 组件验收 / 真机系统集成。
- 候选方案：方案 A 保留 P3-3 完成前的最小实机门禁，P3-5 保留真实后端候选链和十次断连续传；方案 B 把全部安装和真机证据转交 P3-5，并正式降低 P3-3 完成含义为软件组件验收。
- 选择及理由：采用方案 A。不降低现有卡面门槛，尽早验证真实平台 BLE/设备交互；明确 P3-3 可先执行本卡真机验收，避免把 P3-5 当作唯一取得真机证据的阶段。
- 状态：`DECIDED`。
- 授权裁定：用户于 2026-09-10 授权本会话决策并解决，原话见记录 10；Codex 据该授权选择 A 并落盘，不冒称用户逐字指定了 A。
- 决策责任人：用户授权的治理裁定会话 Codex；具体正式合同仍由非实现验收会话按执行合同准备、审批和执行。
- 受影响任务：`P3-3`, `P3-5`。
- 最终落盘目标：`PLAN-OTA-EXEC.md` 两卡验收条款、`docs/ota-prompts/prompt-P3-3-implementation.md` 的“验收归属与实机证据”、`docs/ota-prompts/prompt-P3-5-integration.md` 的“与 P3-3 的验收边界”。
- 证据边界：P3-3 完成前取得同一受验 Actions APK 的实际安装，以及 toy/真包各至少一轮手机到真实 MCU 的完整升级、重启重连和 GET_INFO 目标版本/raw SHA 一致证据；包必须可启动，不把 4KB golden vector、fake 成功或 END ACK 单独当作升级成功。
- 依赖边界：不改变 P3-1/P3-2 的实施前置；P3-3 可经正式合同审批使用合同一致的受控 HTTP 外部输入，不宣称真实 P4-2 链通过。P3-5 保留 P3-3/P3-4/P4-2 等既有依赖、真实后端链和 10/10 独立断连事件，不能把普通成功传输充数。
- 权限边界：本条不授予操作权限或追加配额，不创建验收合同、不填 PASS、不授权安装/卸载、清数据、部署、烧录或其他真机操作；已有开发自测授权不变。具体资产、成功观察和操作计划须纳入本轮正式合同，R2 裁定结束不等于任何产品或实机判据通过。
- 落盘与验证记录：`docs/ota-exec-notes/P3-3-acceptance-scope-ruling-2026-09-10.md`。旧 readiness/research 记录保留，另追加 R2 已裁定的后续说明。

## OTA-DEC-014 BLE 性能诊断与 Android 后台 OTA/通知

- 问题描述：用户反馈 BLE OTA 传输慢、切后台不能继续，并要求状态栏进度。现有源码有逐写分片服务发现热点及主动后台暂停；前者未经实测定因，后者是 P3-3 的既有安全行为，不应通过删保护或篡改旧验收记录处理。
- 所属领域：BLE 性能 / Android 执行生命周期 / OTA 进度 / 任务治理。
- 选择及理由：P3-4 先分阶段测量、按证据做最小热点优化，再 AT 调参；新增 P3-8 实现前台服务实际接管、后台/锁屏继续及通知栏进度。两项分别实施和取证，不能用 Actions 构建结果代替实机吞吐或后台可靠性。
- 状态：`DECIDED`。
- 用户裁定：用户于 2026-09-15 要求把上述需求写入项目规范，原话见记录 11。本会话细化接管、回退和进度边界，不冒称用户逐字指定了具体 API、提速倍数或硬件实验额度。
- 决策责任人：用户提出并批准需求落盘；Codex 负责本次非产品实现的治理细化。后续实现和独立验收仍分离。
- 受影响任务：`P3-4`, `P3-8`。
- 最终落盘目标：跨系统合同 `OTA-XC-BLE-PERFORMANCE`、`OTA-XC-ANDROID-OTA-BACKGROUND`、`OTA-XC-OTA-PROGRESS`，以及看板、P3-4/P3-8 Spec、根/App AGENTS 与治理回归。
- 兼容边界：P3-3-v9、历史冻结包和原实现认领不变；原 `OTA-XC-BLE-TUNING` 数值门槛不变。只把当前共享规范的“后台不得盲写”细化为可靠接管后继续、未接管/失效则回退，不把普通通知视为运行保障。
- 依赖边界：P3-8 依赖已完成 P3-3，不新增 P3-4 -> P3-8 或 P3-8 -> P3-5 硬依赖；P3-5 的既有前置与 10/10 断连续传要求不变。共享文件修改须串行协调。
- 权限边界：只批准需求、规范和治理回归；不新增提交/推送/合并、发布部署、AT 改速、安装卸载、OTA、J-Link 或烧录权限，不沿用 P3-3 的临时额度。开发 CI 权限继续以执行合同 §7.3.2 为准。
- 验证边界：本批规范回归不证明功能已经实现；P3-4/P3-8 尚未观测项保持 NOT_RUN，后续正式合同必须单独审批、冻结并通过执行前检查。
- 落盘与验证记录：`docs/ota-exec-notes/P3-4-P3-8-requirements-2026-09-15.md`。
...
### 记录 10（2026-09-10，验收归属授权裁定）

用户在提交 P3-3/P3-5 两种验收归属方案及矛盾分析后，明确授权：

~~~text
如上是验收agent发现的一个问题，应该怎么做？请你决策并解决该问题，我提前授予你权限
~~~

本会话在上述授权下选择方案 A，形成 OTA-DEC-013；这不是把方案建议冒充用户已选答案，也不追溯修改记录 9。批准的是任务边界的决策与文档修正，具体版本化验收合同、执行授权及未取得证据仍按既有规则处理。
...
~~~text
还有当前ble传输太慢。ota时不能挂后台，通过状态栏显示进度，这两个问题可以解决吗？
~~~

本会话随后说明：先测服务发现/GATT/ACK/staging 再调速；Android 采用前台服务接管和
通知进度，但保留接管失败回退与最终身份复核；提速结合 P3-4，后台 OTA 单独立功能卡。
这是方案说明，不是用户原话。用户接着要求：

~~~text
那么请你写入项目规范
~~~

上述授权用于 OTA-DEC-014 的需求与任务边界落盘，不代表已批准具体实机操作或追加额度。
