...
v1 派工时把「被接入脚本」定为三条 `p3_1_verify_*.py`。第 1 轮独立验收发现
`tests/ota/test_ota_device_info.py`（P3-2 收口引入的产品回归）同样无人执行，
而它与本卡是同一个 workflow 步骤、同一份 `paths` 列表的问题。用户裁定另立新卡
会让计划臃肿，故按同一 `task_id` 出 v2、版本加一、用 `parent_spec_sha256`
绑定 v1 字节，增量登记在看板 §9 变更登记表。

v2 相对 v1 的实质增量共四项：

1. 接线由 3 条扩为 4 条，新增 `tests/ota/test_ota_device_info.py`。
2. `paths` 闭包判据改为「覆盖关系须实算」：新增脚本已被既有通配
   `tests/ota/test_ota_*.py` 覆盖，**不得**为它再加精确条目（加了会破坏
   两份列表的互镜，也会把改动扩散到被冻结合同按路径绑定的范围）。
3. fail-closed 反证由 3 次扩为 4 次，并新增选点安全性的硬性前置判定（见
   「必须新增或调整的测试」）。
4. 触发器反证由 1 条扩为 2 条：新增精确条目一条、既有通配一条。
...
把 P3-6 遗留的三条 `p3_1_verify_*.py` 与 P3-2 收口引入的
`tests/ota/test_ota_device_info.py`，合计**四条产品回归**接入 `.github/workflows/firmware-build.yml` 的 host 测试执行步骤，并在同一批次内扩 `paths` 触发器使这些脚本自身的改动能触发该 workflow，同时把该步骤的名称与其实际执行内容对齐。三件事必须同批完成：只接执行而不扩触发器，等于把门禁改成「改它自己不触发它自己」的静默缺口，正是本卡要消除的东西。
...
- `tests/ota/test_ota_device_info.py`（v2 新增）：驱动 C 用例校验 GET_INFO 设备身份域，失败时**不打印 `=FAIL` 汇总行**（C 侧 `failures>0` 返回 1，Python 侧`check=True` 抛出，`_ALL=PASS` 不再出现）。其文件名已被既有 `paths` 通配`tests/ota/test_ota_*.py` 覆盖，无须新增精确条目。
...
- 覆盖关系必须**实算**，不接受声明。若某条脚本已被既有通配覆盖，就不得为它再加精确条目：冗余条目会破坏两份列表的互镜关系，也会把本卡改动扩散到 `P3-1-v2` 与 `P3-6-v1` 两份冻结合同按路径绑定的范围。新增精确条目只用于现有模式确实覆盖不到的脚本，且不得顺带命中任何**不接入**的脚本。
...
- 三条脚本本身不新增判据；本卡的「测试」是对接线与触发器闭包的验证：
  ...
  - 触发器反证（v2 起为两条）：A 只改一条**由本卡新增精确条目**覆盖的脚本，B 只改一条**由既有通配**覆盖的脚本，各推一次，确认 workflow 均被触发。两条缺一不可——「本仓库自实现的匹配器判自己通过」不构成对 GitHub 匹配行为的证明，而两类覆盖来源必须各自被远端验证一次。若不触发，即说明 `paths` 闭包未成立。
  - fail-closed 反证（v2 起为四次）：对四条脚本各自注入一次可定界缺陷（合同对齐、文本隔离、可移植性、设备身份），确认 workflow 变红且红在对应那一行，随后还原并确认转绿。仅给正例不算完成。
  - **选点安全性是硬前置**。注错必须先证明缺陷不会让**更早的命令或更早的 CI 步骤**先红，否则目标行根本没执行，反证空转：
    1. 命令级：按 CI 同序在本地跑满全部执行行，红点必须恰落在目标行、其前全绿。
    2. **步骤级**：还必须判定该缺陷不会打红本步骤之前的任何 CI 步骤（本 workflow
       中最典型的是固件构建步骤）。本地按命令同序跑一遍**看不见**固件编译，
       因此凡改动会进入编译的产品源，必须另跑一次与 CI 同配置的本地构建来证明
       编译不受影响；无法证明时改用不参与编译的阳性样本（例如用 `#if 0` 包裹的
       文本样本：预处理器直接丢弃，文本扫描器照样命中）。
    3. 目标脚本的红形态必须逐条确认，不得照抄其他脚本。已知 `test_ota_device_info.py`
       失败时没有 `=FAIL` 汇总行，照抄前三条的写法会构造出恒真的空转判据。

## 完成判据

- workflow 的 host 测试步骤显式执行四条脚本，且步骤 `name` 已能涵盖其实际执行内容（boot 向量、OTA host 测试与 P3-1 产品回归）。
- `push` 与 `pull_request` 两份 `paths` 列表都覆盖四条被接入脚本的路径（覆盖来源可以是本卡新增的精确条目或既有通配，但须实算），且两份内容一致。
- 至少一次真实 CI 运行日志显示四条脚本的实际执行输出与结论标记，计数与 `docs/ota-exec-notes/P3-7-card-creation.md` §3 的本机基线一致。
- 真实 CI 证据合计 **11 条腿**：正例 1 + 触发器反证 2 + 注错红 4 + 还原绿 4。每条腿的 run 标识、提交 SHA 与关键日志片段均已留证，且每次注错/还原提交恰好只改一个文件，使红绿因果可归属。
...
执行行在步骤内的排列顺序、步骤名的具体措辞、`paths` 条目用通配模式还是逐个文件名列举（只要满足闭包要求且两份列表同步）、证据笔记的分文件切分方式，以及四处注入缺陷的具体选点（只要各自能被对应脚本确定性捕获、通过「必须新增或调整的测试」一节的命令级与步骤级安全性前置、且可完整还原）。
