...
- 吞吐、错误率、稳定性和统计口径见 `OTA-XC-BLE-TUNING` 与 `OTA-XC-RETRY-POLICY`。实验先按共享候选合同测量，再报告同时满足全部门槛的单一参数组合；产品门槛不从测量结果反向生成。
...
- 共享候选门槛尚未独立复核，或没有单一候选参数组合同时满足全部门槛时，完成有效采集后停止，不宣布生产值。
