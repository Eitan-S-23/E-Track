...
// STM32优化函数属性定义
// Z7_FORCE_INLINE 和 Z7_NO_INLINE 已在 lzma_base_types.h 中定义

// Function type definitions
#define STATIC_FUNC static
#define INLINE_FUNC inline

// Memory alignment and block size definitions
...
// Remove GetUi16 function implementation as it's already defined as a macro
// Z7_FORCE_INLINE UInt16 GetUi16(const void *p)
// {
    ...
    size_t hashSizeSum;
    {
      ...
      if (p->numHashOutBits != 0)
      {
        ...
        const unsigned nbMax =
            (p->numHashBytes == 2 ? 16 : (p->numHashBytes == 3 ? 24 : 32));
        if (numBits > nbMax)
          numBits = nbMax;
        if (numBits >= 32)
          hs = (UInt32)0 - 1;
...
*/
Z7_FORCE_INLINE
UInt32 *Hc_GetMatchesSpec(size_t lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur, CLzRef *son,
