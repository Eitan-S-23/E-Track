...
#define x86_cpuid_MACRO(p, func)                                                        \
  {                                                                                     \
    __asm__ __volatile__(                                                               \
        ASM_LN "mov     %%rbx, %q1" ASM_LN "cpuid" ASM_LN "xchg    %%rbx, %q1"          \
...
#define x86_cpuid_MACRO(p, func)                                                        \
  {                                                                                     \
    __asm__ __volatile__(                                                               \
        ASM_LN "mov     %%ebx, %k1" ASM_LN "cpuid" ASM_LN "xchg    %%ebx, %k1"          \
        : "=a"((p)[0]), "=&r"((p)[1]), "=c"((p)[2]), "=d"((p)[3]) : "0"(func), "2"(0)); \
  }
...
#define EFALGS_CPUID_BIT 21
  UInt32 a;
  __asm__ __volatile__(
      ASM_LN "pushf" ASM_LN "pushf" ASM_LN "pop     %0"
      // ASM_LN   "movl    %0, %1"
...
void __declspec(naked) Z7_FASTCALL z7_x86_cpuid(UInt32 p[4], UInt32 func){
    UNUSED_VAR(p)
        UNUSED_VAR(func) __asm push ebx __asm push edi __asm mov edi,
    ecx // p
    __asm mov eax,
    edx // func
        __asm xor
...
#if defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 1100) || defined(_MSC_VER) && (_MSC_VER >= 1600) && (_MSC_FULL_VER >= 160040219) || defined(__GNUC__) && (__GNUC__ >= 9) || defined(__clang__) && (__clang_major__ >= 9)
