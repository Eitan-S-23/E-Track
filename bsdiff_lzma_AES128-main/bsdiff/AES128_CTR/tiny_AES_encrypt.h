#ifndef TINY_AES_ENCRYPT_H
#define TINY_AES_ENCRYPT_H
...
/**
 * @brief AES-CTR模式加密/解密函数
 * 
 * 在CTR模式中,加密和解密操作相同
 * 
 * @param ctx AES上下文,包含扩展密钥
 * @param buffer 需要加密的数据缓冲区,操作完成后将保存加密结果
 * @param length 数据长度(字节)
 * @param nonce_counter 随机数计数器(16字节),会被修改
 */
void AES_CTR_encrypt(AES_ctx *ctx, uint8_t *buffer, size_t length, uint8_t *nonce_counter);

/**
 * @brief AES单块加密函数
 * 
 * 对单个128位(16字节)块进行AES加密
 * 
 * @param ctx AES上下文,包含扩展密钥
