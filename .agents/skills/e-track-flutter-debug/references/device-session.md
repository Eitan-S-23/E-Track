...
are inputs to inspect, never current authority or reusable action quotas.
...
spending another installation/dispatch reservation.
...
remaining scope and starting a fresh owned runtime. Host recovery never replenishes
an OTA, reset, flash or installation quota.
...
2. During transfer: one invocation; no manual retry or injected disconnect unless
