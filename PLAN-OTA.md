...
> 版本:**v1.3.3(2026-07-26,P0-6 RAM/overlay 回填)**。历程:v1.0 → v1.1 自审 12 项 → v1.2 A 组 8 项重写 → v1.2.1 codex 18 条 → v1.2.2 修 R3 五高危(R4 同意)→ v1.2.3 C 组 10 条 → v1.2.4 修 R6 六高危 → v1.2.5 修 R7 阻断二项 → **R8 codex 复审"同意收敛"**,其 5 条实现验收项已固化进 §2.3/§5.1/§8-P4 → **v1.3.1:version_code 编码重定义(补充 D)** → **v1.3.2:RAM 基线口径修正(补充 C)** → **v1.3.3:P0-6 实测 RAM 基线、16KB 字典与显式 `.sram_ext` overlay 裁决**。**v1.3 为 P0 契约冻结基线**;v1.3.1/v1.3.2/v1.3.3 为看板授权的契约补丁。"同意收敛"指方案层面(codex 未跑构建/真机),实现与生产发布按 §8 各阶段验收。v1 威胁模型(不防主动伪造)为已接受的产品风险(§0.2)。
...
## 9. 风险与开口项
- **RAM 基线(P0-6 实测回填 + P2-6 当前候选回填)**:权威总量 = **EOPB0 扩展后 512KB** = 主 `RAM` 352KB + `RW_IRAM2`/`.sram_ext` 160KB。P0-6 legacy 基线为 AC5 `313528B/360448B`、余 `46920B`;GCC Release `286232B/360448B`、余 `74216B`。P2-6 于 2026-08-19 的 fresh GCC 生产 map(哈希 `20794BC9...914C4BA`)按新冻结口径以 `__StackGuardStart=0x20055FE0` 为静态保留边界:高水位 `352224B/360448B`,其上 `8224B` 恰为 `32B` guard + `8192B` 契约栈;`._user_heap_stack` 末为 `0x200552C0`,到 guard 的可增长余量 `3360B`。P2-6 前当前产物旧口径为 `0x200562C0=352960B`,数值差 `-736B`,但因旧值把 4KiB 栈空洞计入 `._user_heap_stack`，不可解释成通用 RAM 增益。AC5 auxiliary 当前高水位 `351504B`,armlink `Max=360440B`,余 `8936B`,`Program Size: Code=301276 RO-data=289372 RW-data=1332 ZI-data=532248`(map 哈希 `0DC825F0...1C8C935`)。`.sram_ext`/`RW_IRAM2` 仍为 `163840B/163840B`。完整产物哈希、时间戳与算式见 `docs/ota-exec-notes/P2-6-implementation-evidence-2026-08-15.md`。
- **`.sram_ext` 升级独占 overlay(裁决 = A,采纳)**:升级解密/解压/bspatch 前进入显式 `OTA_EXCLUSIVE` 所有权,暂停 LiveMap、关闭刷新/地图文件并使 `snapshotBuf` 失效;同一物理区 `[0x20058000,0x20080000)` 只能由 LiveMap 的 `.sram_ext` 或 OTA 的 `.ota_overlay` 其中一方占用,禁止隐式 malloc/别名挪用。失败/取消时擦除 OTA 池、释放所有权、重新初始化 LiveMap(不得恢复旧快照字节);成功时清理后重启。GCC/AC5 linker 必须把两者放在同一 160KB NOLOAD overlay 并断言不超界,分配失败直接终止 OTA,不得退回无界主堆。完整触发、互斥、恢复、段边界和错误规则见 `docs/ota-binary-contracts.md` §10。
- **升级态峰值预算(P0-6 设计上限,P2-6 真机闭环)**:保留 `lc=2,lp=0,pb=0` 的 **16KB** 字典。已知工作集 `35492B`(CLzmaDec 100 + 概率表 10112 + 字典 16384 + 两个 1KB 流式缓冲 + 单个 4KB 活跃窗口 + AES 上下文/计数器 192 + 解析/流状态 512 + hash/allocator **固定设计上限** 2048;该项不是 P0-6 运行时实测值);加对齐/保护量后 OTA overlay 池上限定为 `40960B`(40KiB),160KB 区仍余 `122880B`。主 RAM 另保留 `8192B` OTA 调用栈:AC5 余量至少 `38728B`,GCC 至少 `66024B`。若不采用 overlay,按同一 `5468B` 对齐/保护量计算,AC5 的 16KB 字典会超 `2232B`;8KB 仍余 `5960B`但牺牲压缩余量,故不选。P2-6 宿主容量边界于 2026-08-19 实测 FULL `prefix=6576B,P_full=33072B`、PATCH `prefix=7640B,P_full=21848B`,均小于 `40960B`;各自 `P_full-prefix` 成功、再减 1B 失败，失败前 candidate prepare/program 均为 `0/0`。这些数值证明固定池诊断与边界鉴别力，不替代 C1/C2 真机生产路径观测。J-Link 因 `Failed to initialized DAP` 未烧录，故真机 workspace/StackInfo/sbrk/TLSF/LiveMap 仍 `NOT_OBSERVED`;C3 静态路线和 C13 measurement-validity 仍有 `EVIDENCE_GAP`，P2-6 保持进行中，不触发字典降档或门槛变更。完整矩阵见 `docs/ota-exec-notes/P2-6-implementation-evidence-2026-08-15.md`。
- **升级态峰值实测回填(P2-6 真机闭环,2026-09-01)**:本条取代上一条尾部「J-Link 因 `Failed to initialized DAP` 未烧录，故真机 workspace/StackInfo/sbrk/TLSF/LiveMap 仍 `NOT_OBSERVED`;C3 静态路线和 C13 measurement-validity 仍有 `EVIDENCE_GAP`」的结论——该结论自 2026-09-01 起失效，其余内容(门槛、工作集分项、宿主边界值)全部继续有效且未被改动。**真机 FULL 升级路径**(2026-08-31 P2-6-SD-R8，固件自身 Apply→Stage→复位→boot 消费，App slot `vcode 20800→20801`):overlay workspace 峰值 `33016B`,占 `40960B` 上限的 `80.61%`,余 `7944B`;`arena_peak == workspace_peak`,`failed_request_size=0`,全程恰一条 FULL 测量记录。OTA 调用栈峰值 `3496B`,占 `8192B` 的 `42.7%`,余 `4696B`;`32B` guard 进入与退出均完整。`sbrk` 增量 `0`,LVGL `lv_tlsf_{malloc,realloc,free}` 增量均 `0`,`lv_mem_monitor` 四字段(`free/big/frag/max`)进出净零。**真机 PATCH 升级路径**(2026-08-30 P2-6-SD-R5，已由 `P2-6-v2` 合同 SHA-256 `0332FEE70D7486C41EF53EC184376A069467DD67C8E80E5744B28B33F9471F62` 冻结):workspace 峰值 `21792B`,占 `40960B` 的 `53.2%`,余 `19168B`;guard 完整、`sbrk` 与 TLSF 三项增量同样为 `0`。**静态栈闭合**(§4.9-E 路线):线程峰值取两条路径的较大者 `2048B`(FULL `1904B`/PATCH `2048B`),加最坏中断预算 `768B`,总上界 `2816B`,占 `8192B` 的 `34.4%`,余 `5376B`;扫描点自身开销 `16B`、测量包装函数上界 `40B` 分别独立报告。**RAM 布局**:生产与测试两构型的三个 OTA 段逐字段一致(`.ota_stack_guard=0x20055FE0`/`32B`、`.ota_stack=0x20056000`/`8192B`、`.ota_overlay=0x20058000`/`40960B`),插桩增量仅 `176B`,`.sram_ext` 仍为 `163840B`。**宿主容量判别**保持 2026-08-19 的 FULL `prefix=6576B,P_full=33072B`、PATCH `prefix=7640B,P_full=21848B`,并在 `26496/26495` 与 `14208/14207` 处各自 ±1B 翻转。真机 FULL 峰值比宿主 `P_full` 低 `56B`,方向与量级同宿主模型一致、未反超门槛;该 `56B` 差值未逐项定界,不作为任何门槛或分项的依据。**结论:预算表闭环——全部实测 ≤ P0-6 预算,不触发 16KiB→8KiB 字典降档,`40960B`/`8192B`/`16KiB`/`35492B`/`5468B` 五个冻结数字一个未改。** C9(AC5 辅助工具链)按冻结提示词保持 `NOT_OBSERVED` 且不阻塞。证据绑定:合同 `docs/acceptance-contracts/P2-6-v3.contract.json` SHA-256 `DA78DDF5BDCA97E1848522193F4FE9F4E7D8E6AC02B0471DFB8F45D9B8AC19B4`,证据矩阵轮次 `P2-6-V3-FREEZE-20260901-01`(27 判据全 `EXECUTED`/`PASS`,无 `REUSED`),`python Tools/acceptance/validate_bundle.py` 退出码 `0`;原始执行记录见 `docs/ota-exec-notes/P2-6-SD-R8-hw-execution-evidence-2026-08-31.md` 与 `P2-6-SD-R9-hw-execution-evidence-2026-09-01.md`。
- BLE 吞吐待实测,协议已留提速参数位。
