...
## 9. 风险与开口项
- **RAM 基线(P0-6 实测回填)**:权威总量 = **EOPB0 扩展后 512KB** = 主 `RAM` 352KB + `RW_IRAM2`/`.sram_ext` 160KB。当前 AC5 map 的 `RW_IRAM1` 高水位为 `313528B/360448B`(86.98%),余 `46920B`(45.82KiB);GCC Release map 的主 RAM 高水位为 `286232B/360448B`(79.41%),余 `74216B`(72.48KiB)。两条 map 均显示 `.sram_ext`/`RW_IRAM2` `163840B/163840B`(100%),由 `snapshotBuf` 静态占满。map、构建产物哈希与计算摘录见 `docs/ota-exec-notes/P0-6-ram-baseline-overlay.md`。
