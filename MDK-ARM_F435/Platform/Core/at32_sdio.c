...
static sd_error_status_type last_error = SD_OK;
static uint8_t last_cmd = SD_CMD_NO_CMD;
...
{
  return sd_card_info.sd_scr_reg.sd_bus_width;
}

uint8_t sd_scr_spec_get(void)
{
  return sd_card_info.sd_scr_reg.sd_spec;
}

uint32_t sd_scr_raw_get(uint8_t index)
{
  if (index < 2) {
    /* note: the data is stored in parsed format in sd_scr_reg structure */
    /* reconstruct the raw data for debugging */
    if (index == 0) {
      /* first 32-bit word */
      return ((uint32_t)sd_card_info.sd_scr_reg.sd_spec) |
             (((uint32_t)sd_card_info.sd_scr_reg.sd_security) << 4) |
             (((uint32_t)sd_card_info.sd_scr_reg.sd_bus_width) << 8) |
             (((uint32_t)sd_card_info.sd_scr_reg.data_stat_after_erase) << 15);
    } else {
      /* second 32-bit word */
      return (((uint32_t)sd_card_info.sd_scr_reg.sd_spec4) << 2) |
             (((uint32_t)sd_card_info.sd_scr_reg.sd_ex_security) << 3) |
             (((uint32_t)sd_card_info.sd_scr_reg.sd_spec3) << 7) |
             ((uint32_t)sd_card_info.sd_scr_reg.cmd_support);
    }
  }
  return 0;
}
...
sd_error_status_type sd_init(void)
{
  ...
  /* configure pc.0, pc.1, pc.2, pc.3, pc.4 pin: d0, d1, d2, d3, clk pin */
  ...
  /* configure pc.5 cmd line */
  gpio_init_struct.gpio_pins = GPIO_PINS_5;
  gpio_init_struct.gpio_pull = GPIO_PULL_UP;
  gpio_init(GPIOC, &gpio_init_struct);

  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE0, GPIO_MUX_10);
  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE1, GPIO_MUX_10);
  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE2, GPIO_MUX_10);
  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE3, GPIO_MUX_10);
  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE4, GPIO_MUX_13);
  gpio_pin_mux_config(GPIOC, GPIO_PINS_SOURCE5, GPIO_MUX_13);
  ...
  while(retry--){
    ...
    delay_ms(10);  // Small delay between retries
  }

  if(status != SD_OK)
  {
    last_error = status;
    return status;  // Return early if power on failed
  }
  ...
  {
    last_error = status;
    return status;  // Return early if card init failed
  }
  ...
  {
    last_error = status;
    return status;  // Return early if card info get failed
  }
  ...
  {
    /* set normal working frequency (48MHz: sdio_ck = 288M/(4+2) = 48MHz) */
...
sd_error_status_type sd_power_on(void)
{
  ...
  /* wait for power and clock to stabilize */
  delay_ms(50);
  ...
  for(retry = 0; retry < 5; retry++)
  {
    ...
    last_cmd = SD_CMD_GO_IDLE_STATE;

    /* get command status */
    status = command_error();
    delay_ms(1);  /* small delay between retries */
  }
  ...
  last_response = sdio_response_get(SDIOx, SDIO_RSP1_INDEX);
  ...
  if(SD_OK == status)
  {
    ...
    {
      /* delay between retries for card to process */
      delay_ms(10);
      ...
      /* enable ccsm */
      sdio_command_state_machine_enable(SDIOx, TRUE);
      last_cmd = SD_CMD_SD_APP_OP_COND;
...
sd_error_status_type sd_mult_blocks_read(uint8_t *buf, long long addr, uint16_t blk_size, uint32_t nblks)
{
  ...
  /* 本工程未使能 SDIO 的 NVIC 中断，sd_irq_service 中 stop_flag==1 的停卡
     路径永远不会执行（自检实验实锤：CMD18 数据正常收完但卡停留在 data 态，
     后续命令全部失败）。因此这里不依赖中断，置 stop_flag=0，改为在数据
     完成后同步发送 CMD12。 */
  stop_flag = 0;
  ...
  /* send cmd12, stop transmission: bring the card back to tran state */
  sdio_command_init_struct.argument = 0;
  ...
  return command_rsp1_error(SD_CMD_STOP_TRANSMISSION);
}
...
sd_error_status_type scr_find(void)
{
  ...
  while(!(sts_reg & (SDIO_RXERRO_FLAG | SDIO_DTFAIL_FLAG | SDIO_DTTIMEOUT_FLAG | SDIO_DTBLKCMPL_FLAG | SDIO_SBITERR_FLAG)) && timeout)
  {
    if(sdio_flag_get(SDIOx, SDIO_RXBUF_FLAG) != RESET)
    {
      if(index < 2)
      ...
      timeout = SDIO_DATATIMEOUT; /* reset timeout when data received */
    }
    else
    {
      timeout--;
  ...
  /* Verify that we read the expected 2 words */
  if(index < 2)
  {
    return SD_DATA_FAIL;
  }

  /* Parse SCR register according to SD specification */
  /* SCR format (64 bits, big-endian transmission):
   * Bits 63:60 - SCR_STRUCTURE
   * Bits 59:56 - SD_SPEC
   * Bits 55    - DATA_STAT_AFTER_ERASE
   * Bits 54:52 - SD_SECURITY
   * Bits 51:48 - SD_BUS_WIDTHS
   * Bits 47    - SD_SPEC3
   * Bits 46:43 - SD_EX_SECURITY
   * Bits 42    - SD_SPEC4
   * Bits 33:32 - CMD_SUPPORT
   */

  uint8_t *scr_bytes = (uint8_t *)tempscr;

  /* Byte 0 (bits 63-56): SCR_STRUCTURE[7:4] | SD_SPEC[3:0] */
  sd_card_info.sd_scr_reg.sd_spec = (scr_bytes[0] & 0x0F);

  /* Byte 1 (bits 55-48): DATA_STAT[7] | SD_SECURITY[6:4] | SD_BUS_WIDTHS[3:0] */
  sd_card_info.sd_scr_reg.data_stat_after_erase = (scr_bytes[1] >> 7) & 0x01;
  sd_card_info.sd_scr_reg.sd_security = (scr_bytes[1] >> 4) & 0x07;
  sd_card_info.sd_scr_reg.sd_bus_width = (scr_bytes[1] & 0x0F);

  /* Byte 2 (bits 47-40): SD_SPEC3[7] | SD_EX_SECURITY[6:3] | SD_SPEC4[2] */
  sd_card_info.sd_scr_reg.sd_spec3 = (scr_bytes[2] >> 7) & 0x01;
  sd_card_info.sd_scr_reg.sd_ex_security = (scr_bytes[2] >> 3) & 0x0F;
  sd_card_info.sd_scr_reg.sd_spec4 = (scr_bytes[2] >> 2) & 0x01;

  /* Byte 4 (bits 33-32 in upper 2 bits): CMD_SUPPORT */
  sd_card_info.sd_scr_reg.cmd_support = (scr_bytes[4] >> 6) & 0x03;
...
/**
  * @brief  dma2 channel4 interrupt handler (for SDIO)
  * @param  none
  * @retval none
  */
void DMA2_Channel4_IRQHandler(void)
{
  ...
  if(dma_flag_get(DMA2_HDT4_FLAG) != RESET)
  {
    dma_flag_clear(DMA2_HDT4_FLAG);
  }
  if(dma_flag_get(DMA2_DTERR4_FLAG) != RESET)
  {
    dma_flag_clear(DMA2_DTERR4_FLAG);
  }
}
